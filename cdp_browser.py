    ...
    try:
        ...
        if scroll_times > 0:
            for _ in range(scroll_times):
                ...
                time.sleep(1.0)
        # 提取帖子链接的 JavaScript
        js_expr = "(function(){var out=[];var seen={};var host=location.hostname.replace(/^www\\./,'');var links=document.querySelectorAll('a');var total=links.length;var sameHost=0;links.forEach(function(a){if(!a.href)return;try{var u=new URL(a.href,location.href);}catch(e){return;}if(u.hostname.replace(/^www\\./,'')!==host)return;sameHost++;var p=u.pathname;if(p==='/'||p==='')return;var clean=u.origin+u.pathname;if(!seen[clean]){seen[clean]=1;out.push(clean);}});return JSON.stringify({total:total,sameHost:sameHost,found:out.length,links:out.slice(0,10)});})()"
        ...
        try:
            ...
            if _debug.get('links'):
                lg('前10个链接: %s' % str(_debug['links'])[:200])
